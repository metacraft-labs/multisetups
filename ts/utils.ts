...
function computeB3sum(filePath) {
    ...
    if (result.code !== 0) {
        console.error(
            `Error computing b3sum for file ${filePath}: ${result.stderr}`
        );
        return null;
    }
...
const getZkeyFiles = (dirname: string): Array<any> => {
    const zkeyFiles: any[] = [];
...
const validateZkeyDir = (dirname: string) => {
...
const countDirents = (dirname: string): number => {
    let numFiles = 0;
